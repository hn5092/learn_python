...
class cg_ali213_reyouwp_list(PythonContentAnalyst):
    ...
    def setConfig(self,url):
        ...
        self.config.cfgContent.Path=["div.win_body_type_left_top_con:eq(1) div.win_body_type_left_top_con_info a","div.andriod_gametj_left div.andriod_gametj_left_con:eq(1) div.andriod_gametj_left_con_pic a","div.iphone_gametj_right div.iphone_gametj_right_con:eq(1) div.iphone_gametj_right_con_pic a"]        
...
def getresult(url):
    contentAnalyst=cg_ali213_reyouwp_list()
...
class TC0(unittest.TestCase):
    def __init__(self, methodName = 'runTest'):
        ...
        self.contentAnalyst=cg_ali213_reyouwp_list()
       
    def testConfig(self):        
        self.contentAnalyst.execute("http://m.ali213.net/ios/?r")
        content=self.contentAnalyst.result.content
        print content,len(content.split("|"))
        self.assertGreater(len(content.split("|")),11)
        pass

        self.contentAnalyst.execute("http://m.ali213.net/android/?r")
        content=self.contentAnalyst.result.content
        print content,len(content.split("|"))
        self.assertGreater(len(content.split("|")),14)
        pass

        self.contentAnalyst.execute("http://m.ali213.net/wp/")
        content=self.contentAnalyst.result.content
        print content,len(content.split("|"))
        self.assertGreater(len(content.split("|")),14)
        pass
